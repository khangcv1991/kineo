...
spl_autoload_register("b13_dashboard_autoload");
function b13_dashboard_autoload($classname) {
    global $CFG;

    if (strpos($classname, 'b13') === false) {
        return;
    }
...
function load_class() {
    ...
    if (strpos($classname, '-')) {
        $class = 'local_b13_' . str_replace('-', '\\', $classname);
    } else {
        $class = 'local_b13_dashboard\\' . $classname;
    }
...
function get_string_b13($identifier, $object = null) {
    return get_string($identifier, 'local_b13_dashboard', $object);
}
