    ...
    public static function create_menu() {
        ...
        $menu .= dashboard_util::add_menu(
            (new menu_util())->set_classname('users')
                ...
                ->set_icon('users')
                ->set_name(get_string_b13('user_title')));
        $menu .= dashboard_util::add_menu(
            (new menu_util())->set_classname('courses')
                ->set_methodname('dashboard')
                ->set_icon('courses')
                ->set_name(get_string_b13('courses_title')));

        $sql = "SELECT plugin
                  FROM {config_plugins}
                 WHERE plugin LIKE 'local\_b13\_%'
                   AND plugin !=   'local_b13_dashboard'
        ...
        $menu .= dashboard_util::add_menu(
            (new menu_util())->set_classname('reports')
                ->set_methodname('dashboard')
                ->set_icon('report')
                ->set_name(get_string_b13('reports_title'))
        ...
        if (has_capability('moodle/site:config', \context_system::instance())) {
            $menu .= dashboard_util::add_menu(
                (new menu_util())->set_classname('notifications')
                    ...
                    ->set_name(get_string_b13('notification_title')));
        }
        ...
        if (has_capability('moodle/site:config', \context_system::instance())) {
            $menu .= dashboard_util::add_menu(
                (new menu_util())->set_classname('webpages')
                    ->set_methodname('dashboard')
                    ->set_icon('webpages')
                    ->set_name(get_string_b13('webpages_title')));
        }
        ...
        $menu .= dashboard_util::add_menu(
            (new menu_util())->set_classname('benchmark')
                ->set_methodname('test')
                ->set_icon('performace')
                ->set_name(get_string_b13('benchmark_title')));
