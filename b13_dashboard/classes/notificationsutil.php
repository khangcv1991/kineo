...
use local_b13_dashboard\util\release;
...
class notificationsutil {
    ...
    public function add_form_extra() {
        ...
        if (!isset($module[1])) {
            end_util::end_script_show(get_string_b13('notification_add_selectmodule'));
        }
        ...
        $form->add_input(
            input_select::new_instance()
                ->set_title(get_string_b13('notification_add_action'))
                ->set_name('event')
                ->set_values($eventslist)
                ->set_add_selecione(true));
        $form->create_submit_input(get_string_b13('notification_add_create'));
    }
    ...
    public function settings_load_template() {
        ...
        $templatefile = "{$CFG->dirroot}/local/b13_dashboard/assets/mail/{$template}";

        $content = file_get_contents($templatefile);

        $linkmanager
            = "<a href=\"{$CFG->wwwroot}/local/b13_dashboard/open-dashboard.php?classname=notifications&method=dashboard\"
                  target=\"_blank\" style=\"border-bottom:1px #777777 dotted; text-decoration:none; color:#777777;\">
                            " . get_string_b13('notification_manager') . "
                        </a>";
    ...
    protected function module_name($component, $onlyused) {
        ...
        switch ($component) {
            case 'core_course_category':
                return get_string_b13('notification_core_course_category');
            case 'core_course':
                return get_string_b13('notification_core_course');
            ...
            case 'core_user':
                return get_string_b13('notification_core_user');
            case 'core_user_enrolment':
                return get_string_b13('notification_core_user_enrolment');
            case 'core_user_password':
                return 'Senhas';

            case 'local_b13_dashboard':
                return get_string_b13('notification_local_b13_dashboard');
            case 'local_b13_hotmoodle':
                return get_string_b13('notification_local_b13_hotmoodle');
            case 'local_b13_moocommerce':
                return get_string_b13('notification_local_b13_moocommerce');
            case 'local_b13_pay':
                return get_string_b13('notification_local_b13_pay');
        }
    ...
    public static function mensagem_no_smtp() {
        ...
        if (release::version() < 3.2) {
            ...
            mensagem::print_danger(get_string_b13('notification_error_smtp', $CFG));
        } else {
