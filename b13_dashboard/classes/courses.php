...
class courses {
    ...
    public function dashboard() {
        dashboard_util::add_breadcrumb(get_string_b13('courses_title'));
        dashboard_util::add_breadcrumb(get_string_b13('courses_title1'));
    ...
    public function details() {
        ...
        if ($courseid == 0) {
            header::notfound(get_string_b13('courses_invalid'));
        }

        $course = $DB->get_record('course', array('id' => $courseid));
        header::notfound_null($course, get_string_b13('courses_notound'));

        dashboard_util::add_breadcrumb(get_string_b13('courses_title'), '?classname=courses&method=dashboard');
        dashboard_util::add_breadcrumb($course->fullname);
        dashboard_util::start_page();

        echo '<div class="element-box">
                  <h3>' . get_string_b13('courses_sumary') . '
                      ' . button::info(get_string_b13('courses_edit'), $CFG->wwwroot . '/course/edit.php?id=' .
                $course->id . '#id_summary_editor', button::BTN_PEQUENO, false, true) . '
                      ' . button::primary(get_string_b13('courses_acess'), $CFG->wwwroot . '/course/view.php?id=' .
        ...
        $table->add_header(get_string_b13('courses_student_name'), 'nome');
        $table->add_header(get_string_b13('courses_student_email'), 'email');
        $table->add_header(get_string_b13('courses_student_status'), 'status', table_header_item::RENDERER_STATUS);
    ...
    public function create_static_page($course) {
        ...
        if ($menus) {
            ...
            $webpagess = $DB->get_records('b13_dashboard_webpages', array('courseid' => $course->id));
            ...
            $form->create_hidden_input('theme', get_config('local_b13_dashboard', 'webpages_theme'));
            $form->create_hidden_input('text', $course->summary);
            $form->create_hidden_input('visible', 1);
            $form->create_submit_input(get_string_b13('courses_page_create'), 'margin-left-15');
            $form->close();
            echo '</div>';
        }
    }
