...
use local_b13_dashboard\html\table;
use local_b13_dashboard\util\bytes_util;
...
class backup {
    ...
    public function dashboard() {
        dashboard_util::add_breadcrumb(get_string_b13('backup_title'));
        dashboard_util::start_page();

        echo '<div class="element-box">';

        if (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') {
            mensagem::print_warning(get_string_b13('backup_windows'));
        } else {
            if (server_util::function_enable('shell_exec')) {
                mensagem::print_info(get_string_b13('backup_hours'));
                echo '<div style="text-align: center;">
                          <p>' . get_string_b13('backup_sleep') . '</p>';

                button::add(get_string_b13('backup_newsqlnow'), '?classname=backup&method=execute_dumpsql');

                echo '</div>';
            } else {
                mensagem::print_danger(get_string_b13('backup_noshell'));
            }
            ...
            if (isset($backupslista[0])) {
                ...
                $table = new table();
                $table->add_header(get_string_b13('backup_list_file'), 'file');
                $table->add_header(get_string_b13('backup_list_created'), 'data');
                $table->add_header(get_string_b13('backup_list_size'), 'size');
                $table->add_header(get_string_b13('backup_list_action'), 'acoes');

                $table->set_row($backupslista);
                $table->close(true, 'order:[[1,"DESC"]]');
            } else {
                mensagem::print_warning(get_string_b13('backup_none'));
            }
        }
        echo '</div>';
    ...
    public function execute_dumpsql() {
        ...
        dashboard_util::add_breadcrumb(get_string_b13('backup_title'), '?classname=backup&method=dashboard');
        dashboard_util::add_breadcrumb(get_string_b13('backup_execute_exec'));
        dashboard_util::add_breadcrumb(get_string_b13('backup_execute_exec') . ": {$CFG->dbname}");
        dashboard_util::start_page();
        ...
        $dumpstart = "--" . get_string_b13('pluginname') . " SQL Dump\n-- Host: {$CFG->dbhost}\n-- " .
            get_string_b13('backup_execute_date') . " " . date('d/m/Y \à\s H-i') . "\n\n";
        file_put_contents($backupfile, $dumpstart);

        $dbname = "--\n-- " . get_string_b13('backup_execute_database') .
            " `{$CFG->dbname}`\n--";
        ...
        foreach ($tables as $table => $val) {
            ...
            echo "<p id='tabela-dump-$table'>" . get_string_b13('backup_execute_table') . " <strong>$table</strong></p>
                      <script>
                          jQuery( 'html,body' ).animate ( { scrollTop: $( '#tabela-dump-$table' ).offset().top }, 0 );
                      </script>";

            $dbstart = "\n\n\n--\n-- " . get_string_b13('backup_execute_structure') . " `$table`\n--\n\n";
            ...
            if (isset($schema->{'create table'})) {
                ...
                $dbdumpstart = "--\n-- " . get_string_b13('backup_execute_dump') . " `$table`\n--\n";
                ...
                $tablesql = "-- " . get_string_b13('backup_execute_dump_error') . "\n\n";
                file_put_contents($backupfile, $tablesql, FILE_APPEND);
            }
        }

        mensagem::print_success(get_string_b13('backup_execute_complete'));

        echo '<div id="end-page-to" style="text-align: center;">';
        button::add(get_string_b13('backup_returnlist'), '?classname=backup&method=dashboard');
        echo '<script>
    ...
    public function delete() {
        ...
        if (file_exists($backupfile)) {
            ...
            if ($status) {
                ...
                mensagem::agenda_mensagem_success(get_string_b13('backup_deletesucessfull'));
                header::location('?classname=backup&method=dashboard');
            } else {

                dashboard_util::add_breadcrumb(get_string_b13('backup_title'), '?classname=backup&method=dashboard');
                dashboard_util::add_breadcrumb(get_string_b13('backup_deleting'));
                dashboard_util::start_page();

                echo "<div class=\"element-box\">
                          <h3>" . get_string_b13('backup_delete_confirm') . "</h3>
                          <p>" . get_string_b13('backup_delete_title', ['file' => $file, 'data' => $data]) . "</p>
                          <div>";
            ...
            header::notfound(get_string_b13('backup_notound'));
        }
    }
